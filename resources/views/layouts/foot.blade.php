	...
	<!-- Slimscroll js -->
	{!! HTML::script('assets/third/slimscroll/jquery.slimscroll.js') !!}
	...
	{!! HTML::script('assets/third/dropzone/dropzone.js') !!}
    @endif
	...
	$(document).ready(function() { 
		$('.template').on('change', function (e) {
			$.ajax({
	            ...
	            success: function (response) {
	            	...
	            	$('.mail_subject').html(response.title_data);
		...
		Morris.Donut({
		  element: 'ticket-status-stat',
		  colors: {!! json_encode($status_colors) !!},
		  data:  {!! json_encode($status_stats) !!}
		});
		Morris.Donut({
		  element: 'ticket-priority-stat',
		  colors: {!! json_encode($type_colors) !!},
		  data:  {!! json_encode($priority_stats) !!}
		});
		Morris.Donut({
		  element: 'ticket-type-stat',
		  colors: {!! json_encode($type_colors) !!},
		  data:  {!! json_encode($type_stats) !!}
		});
		Morris.Donut({
		  element: 'ticket-department-stat',
		  colors: {!! json_encode($department_colors) !!},
		  data:  {!! json_encode($department_stats) !!}
		});
